<script lang="ts">
  ...
  // prettier-ignore
  const url = {
    hooks: 'https://tb.dev.br/tauri-store/reference/tauri-plugin-svelte/types/StoreHooks.html',
...
<Changelog.BreakingChanges {version}>
  <ul>
    <li>
      Rename <code>getSveltePath</code> to <code>getStoreCollectionPath</code>.
    </li>
    <li>
      Rename <code>setSveltePath</code> to <code>setStoreCollectionPath</code>.
    </li>
