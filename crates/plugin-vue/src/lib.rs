...
#[cfg(target_os = "ios")]
tauri::ios_plugin_binding!(init_plugin_vue_plugin);
...
pub struct Builder<R: Runtime> {
  ...
  on_load: Option<Box<OnLoadFn<R, VueMarker>>>,
  pretty: bool,
...
impl<R: Runtime> Builder<R> {
  ...
  where
    F: Fn(&Store<R, VueMarker>) -> Result<()> + Send + Sync + 'static,
  {
  ...
  fn build_collection(self, handle: Handle<R>) -> Result<()> {
    let mut builder = StoreCollection::<R, VueMarker>::builder()
  ...
  D: DeserializeOwned,
{
  ...
  let handle = api.register_android_plugin("", "VuePlugin")?;
  #[cfg(target_os = "ios")]
  let handle = api.register_ios_plugin(init_plugin_vue_plugin)?;
