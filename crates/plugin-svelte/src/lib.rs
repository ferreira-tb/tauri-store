...
#[cfg(target_os = "ios")]
tauri::ios_plugin_binding!(init_plugin_svelte);
...
pub struct Builder<R: Runtime> {
  ...
  on_load: Option<Box<OnLoadFn<R, SvelteMarker>>>,
...
impl<R: Runtime> Builder<R> {
  ...
  where
    F: Fn(&Store<R, SvelteMarker>) -> Result<()> + Send + Sync + 'static,
  {
  ...
  fn build_collection(self, handle: Handle<R>) -> Result<()> {
    let mut builder = StoreCollection::<R, SvelteMarker>::builder()
  ...
  D: DeserializeOwned,
{
  ...
  let handle = api.register_android_plugin("com.plugin.svelte", "SveltePlugin")?;

  #[cfg(target_os = "ios")]
  let handle = api.register_ios_plugin(init_plugin_svelte)?;
