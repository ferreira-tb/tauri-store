...
where
  R: Runtime,
{
  let valtio = app.valtio();
  ids.iter().for_each(|id| valtio.allow_save(id));
}
...
{
  let valtio = app.valtio();
  ids.iter().for_each(|id| valtio.allow_sync(id));
}
...
{
  let valtio = app.valtio();
  ids.iter().for_each(|id| valtio.deny_save(id));
}
...
{
  let valtio = app.valtio();
  ids.iter().for_each(|id| valtio.deny_sync(id));
}
...
where
  R: Runtime,
{
  app.valtio().with_store(id, |store| store.path())
}
  ...
  R: Runtime,
{
  let app = window.app_handle();
  let label = window.label().to_owned();
  app.valtio().with_store(id, move |store| {
