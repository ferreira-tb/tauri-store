...
#[cfg(target_os = "ios")]
tauri::ios_plugin_binding!(init_plugin_valtio_plugin);
...
pub struct Builder<R: Runtime> {
  ...
  on_load: Option<Box<OnLoadFn<R, ValtioMarker>>>,
  pretty: bool,
...
impl<R: Runtime> Builder<R> {
  ...
  where
    F: Fn(&Store<R, ValtioMarker>) -> Result<()> + Send + Sync + 'static,
  {
  ...
  fn build_collection(self, handle: Handle<R>) -> Result<()> {
    let mut builder = StoreCollection::<R, ValtioMarker>::builder()
  ...
  D: DeserializeOwned,
{
  ...
  let handle = api.register_android_plugin("", "ValtioPlugin")?;
  #[cfg(target_os = "ios")]
  let handle = api.register_ios_plugin(init_plugin_valtio_plugin)?;
