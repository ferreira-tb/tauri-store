...
use crate::command::codegen::util::transform::{
  prepend_autogenerated_hashtag, prepend_autogenerated_xml,
};
...
struct Generator<'a> {
  ...
  #[builder(default = &[&prepend_autogenerated_xml])]
  xml_transform: &'a [&'a dyn Fn(&mut String)],

  #[builder(default = &[&prepend_autogenerated_hashtag])]
  hashtag_transform: &'a [&'a dyn Fn(&mut String)],
...
impl Generator<'_> {
  pub fn generate(self) -> Result<()> {
    for plugin in Plugin::VARIANTS {
      ...
      match self.input.file_name() {
        Some(name) if name == ".gitignore" => {
          self
            .hashtag_transform
            .iter()
            .for_each(|t| t(&mut contents));
        }
        _ => match self.input.extension() {
          Some(ext) if ext == "xml" => {
            self
              .xml_transform
              .iter()
      ...
      (self.output)(ctx).iter().try_for_each(|path| {
