  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| zustand.allow_save(id));
}
  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| zustand.allow_sync(id));
}
  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| zustand.deny_save(id));
}
  ...
  R: Runtime,
{
  ...
  ids
    .iter()
    .for_each(|id| zustand.deny_sync(id));
}
...
where
  R: Runtime,
{
  app.zustand().default_save_strategy()
}
...
where
  R: Runtime,
{
  spawn_blocking(move || app.zustand().set_path(path)).await?
}
